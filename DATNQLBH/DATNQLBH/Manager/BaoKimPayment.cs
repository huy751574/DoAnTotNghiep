...
class BaoKimPayment
{
    ...
    //Mã merchant site
    private int merchant_id = 23315;	//Thay bằng mã merchant site bạn đã đăng ký trên BaoKim.vn

    //Mật khẩu bảo mật
    private string secure_pass = "e52633f58caef27e";	//Thay bằng mật khẩu giao tiếp giữa website của bạn với BaoKim.vn
     ...
     * @return url cần tạo
     */
    public string createRequestUrl(string userid_id, string email, string total_amount, string order_description, string url_success, string url_cancel)
    {
        Hashtable order_params = new Hashtable();
        order_params.Add("merchant_id", this.merchant_id);
        order_params.Add("order_id", userid_id);
        order_params.Add("business", email);
        ...
        var url_detail= "http://qlbh.azurewebsites.net/Account/Error";
